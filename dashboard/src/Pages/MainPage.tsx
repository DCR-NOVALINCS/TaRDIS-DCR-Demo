...
import { addUser, reconfiguration, reorderUsers, stringFy, UserState } from "../Store/users";
...
const mapper = (users: UserState[]): RowData[] => {
  // console.log("Mapping users", users);
  var ret = users.map((user, index) => {
      ...
      <TableContainer component={Paper}>
        <IconButton
          ...
          onClick={() => {
            ...
            //   [...prev, { id: newId, ip: "localhost", port: 0, self: "Unknown", events: [], connection: false }]);
            dispatch(addUser({ ip: "localhost", port: 1234 }));
          ...
          <Droppable droppableId="usersTable">
            {(provided) => (
              <Table {...provided.droppableProps} ref={provided.innerRef}>
                ...
                >
                  {rows.map((row, index) => (
                    <UserEntry key={index} row={row} rows={rows}
